...
int ngspice_exit_callback(int exit_status, bool unload_immediately,
                          bool exit_on_quit, int id, void *user_data);

int ngspice_output_callback(char *output_str, int id, void *user_data);
...
// sends a command to ngspice to alter 'device_name' with the given 'value'
void send_ngspice_alter_cmd(const char *device_name, const char value);
